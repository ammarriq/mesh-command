import * as React from "react";

const EyeSlashIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg
    width={24}
    height={25}
    viewBox="0 0 24 25"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <path
      d="M14.5299 9.97004L9.46992 15.03C8.81992 14.38 8.41992 13.49 8.41992 12.5C8.41992 10.52 10.0199 8.92004 11.9999 8.92004C12.9899 8.92004 13.8799 9.32004 14.5299 9.97004Z"
      stroke="#071437"
      strokeWidth={1.5}
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M17.8201 6.26998C16.0701 4.94998 14.0701 4.22998 12.0001 4.22998C8.47009 4.22998 5.18009 6.30998 2.89009 9.90998C1.99009 11.32 1.99009 13.69 2.89009 15.1C3.68009 16.34 4.60009 17.41 5.60009 18.27"
      stroke="#071437"
      strokeWidth={1.5}
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M8.41992 20.0301C9.55992 20.5101 10.7699 20.7701 11.9999 20.7701C15.5299 20.7701 18.8199 18.6901 21.1099 15.0901C22.0099 13.6801 22.0099 11.3101 21.1099 9.90005C20.7799 9.38005 20.4199 8.89005 20.0499 8.43005"
      stroke="#071437"
      strokeWidth={1.5}
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M15.5099 13.2C15.2499 14.61 14.0999 15.76 12.6899 16.02"
      stroke="#071437"
      strokeWidth={1.5}
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M9.47 15.03L2 22.5"
      stroke="#071437"
      strokeWidth={1.5}
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <path
      d="M22 2.5L14.53 9.97"
      stroke="#071437"
      strokeWidth={1.5}
      strokeLinecap="round"
      strokeLinejoin="round"
    />
  </svg>
);

export default EyeSlashIcon;
