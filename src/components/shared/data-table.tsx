            ...
            {columns.map((column, index) => (
              <TableHead
                ...
                className={cn(
                  ...
                  index === 0 ? 'w-full min-w-[180px]' : 'text-right min-w-[120px]',
                  column.className,
                )}
              >
                {column.header}
          ...
          {data.map((row, rowIndex) => (
            ...
            >
              {columns.map((column, colIndex) => (
                <TableCell
                  key={colIndex}
                  className={cn('py-4', colIndex === 0 ? 'w-full' : 'text-right', column.className)}
                >
                  {column.cell
                    ? column.cell(row)
...
export function InvoiceTable({ invoices, onDownload, onPreview }: InvoiceTableProps) {
  const columns: Column<InvoiceTableRow>[] = [
    ...
    },
    {
      key: 'amount',
      header: 'Amount',
      cell: (row) => <span className="text-sm text-gray-900">USD ${row.amount.toFixed(2)}</span>,
    },
    {
      key: 'actions',
      header: '',
